<template>
  <q-page :class="['map-detail-page', $q.dark.isActive ? 'mdp--dark' : 'mdp--light']">
    ...
    <div v-if="region && regionSegment" class="segment-panel">
      <!-- Left Mondrian accent column -->
      <div class="segment-left-col">
        <div class="slc-block slc-block--red" />
        <div class="slc-sep" />
        <div class="slc-block slc-block--blue" />
        <div class="slc-sep" />
        <div class="slc-block slc-block--yellow" />
        <div class="slc-sep" />
        <div class="slc-block slc-block--black" />
      </div>
      <!-- Content -->
      <div class="segment-content">
        <div class="segment-eyebrow">2026 Summer Tour</div>
        <div class="segment-name">{{ region.name }}</div>
        <div class="segment-text">{{ regionSegment }}</div>
      </div>
      <!-- Right Mondrian accent column -->
      <div class="segment-right-col">
        <div class="slc-block slc-block--yellow" />
        <div class="slc-sep" />
        <div class="slc-block slc-block--black" />
        <div class="slc-sep" />
    ...
    <!-- Mondrian section divider -->
    <div v-if="region" class="mondrian-rule">
      <div class="mr-block" style="background:#cc1010;flex:3" />
      <div class="mr-sep" />
      <div class="mr-block" style="background:#111;flex:1" />
      <div class="mr-sep" />
      <div class="mr-block" style="background:#ffd600;flex:4" />
      <div class="mr-sep" />
      <div class="mr-block" style="background:#1040a8;flex:2" />
      <div class="mr-sep" />
      <div class="mr-block" style="background:#f5f0e8;flex:5" />
      <div class="mr-sep" />
      <div class="mr-block" style="background:#cc1010;flex:2" />
    </div>
    ...
    <!-- Mondrian section divider -->
    <div class="mondrian-rule">
      <div class="mr-block" style="background:#111;flex:2" />
      <div class="mr-sep" />
      <div class="mr-block" style="background:#ffd600;flex:4" />
      <div class="mr-sep" />
      <div class="mr-block" style="background:#cc1010;flex:3" />
      <div class="mr-sep" />
      <div class="mr-block" style="background:#1040a8;flex:5" />
      <div class="mr-sep" />
      <div class="mr-block" style="background:#f5f0e8;flex:2" />
    </div>
...
const regionSegment = ref<string>('')
...
const REGION_HERO_IMAGES: Record<string, string[]> = {
  ...
  'southwest': [
    'https://images.unsplash.com/photo-1703821717473-c512aa0b4654?auto=format&fit=crop&w=1600&q=80', // southwest outdoor festival
  ...
  'northeast': [
    'https://images.unsplash.com/photo-1446329813274-7c9036bd9a1f?auto=format&fit=crop&w=1600&q=80', // autumn forest path
    'https://images.unsplash.com/photo-1533174072545-7a4b6ad7a6c3?auto=format&fit=crop&w=1600&q=80', // festival crowd confetti
...
// ── Per-region tour segment descriptions (Nikki can override via site_settings) ──
const REGION_SEGMENT_DEFAULTS: Record<string, string> = {
  'northeast':       'The Northeast leg winds from the Catskills to coastal Maine — jazz on the Cape, bluegrass in Vermont\'s Green Mountains, and Hudson Valley nights under the stars. A four-week journey through America\'s most storied festival country.',
  'southeast':       'The Southeast leg winds through Appalachian hollows and Gulf Coast shores — Asheville to New Orleans, deep roots music at every stop. Expect hot summer nights, front-porch vibes, and some of the warmest crowds on the tour.',
  'southwest':       'The Southwest leg crosses high desert country under wide-open skies — Red Rock amphitheaters, Santa Fe plaza stages, and canyon-rim venues in Utah and Arizona. Blistering sunsets, cosmic stargazing, and a meeting of folk, country, and native sounds.',
  'mountain-west':   'The Mountain West leg climbs into the Rockies at full summer bloom — altitude festivals in Colorado, Wyoming meadow stages, and Idaho wilderness venues. String bands, jam bands, and everything in between echoing off granite peaks.',
  'pacific-northwest': 'The Pacific Northwest leg moves through old-growth forest country — Eugene outdoor stages, Portland waterfront venues, and Washington coast amphitheaters. Lush, green, and alive with Pacific Coast energy.',
  'great-lakes':     'The Great Lakes leg spans the upper Midwest waterfront — Chicago lakefront stages to Michigan U.P. wilderness festivals. Blues, jazz, and Midwestern folk under the wide-open sky of America\'s inland sea.',
}
...
onMounted(async () => {
  ...
  const [{ data: rgn }, { data: pts }, { data: heroData }, { data: segmentData }] = await Promise.all([
    ...
    supabase.from('site_settings').select('value').eq('key', `region_segment_${regionId}`).limit(1),
  ])

  region.value = rgn as MapRegion

  // Load tour segment description (site_settings key: region_segment_<id>)
  const segVal = (segmentData as { value: unknown }[] | null)?.[0]?.value
  regionSegment.value = (typeof segVal === 'string' && segVal)
    ? segVal
    : (REGION_SEGMENT_DEFAULTS[regionId] ?? '')
...
.mr-block { height: 100%; flex-shrink: 0; }
.mr-sep   { width: 4px; flex-shrink: 0; background: #111; }
...
.segment-left-col  { border-right: 4px solid #111; }
.segment-right-col { border-left:  4px solid #111; }

.slc-block { flex: 1; }
.slc-block--red    { background: #cc1010; }
.slc-block--blue   { background: #1040a8; }
.slc-block--yellow { background: #ffd600; }
.slc-block--black  { background: #111; }
.slc-sep { height: 4px; flex-shrink: 0; background: #111; }
...
.mdp--light {
  ...
  .segment-panel { background: #fff; border-bottom-color: #111; }
  .segment-eyebrow { color: #b07000; }
  .segment-name    { color: #0a0018; }
  .segment-text    { color: rgba(26,10,46,0.6); }
}
</style>
